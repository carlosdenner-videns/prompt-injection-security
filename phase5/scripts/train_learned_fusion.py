"""
Phase 5: Train Learned Fusion with FPR-Constrained Thresholding
Uses stratified 5-fold CV with per-fold threshold calibration to enforce FPR ≤ 1%.
"""
...
class LearnedFusionTrainer:
    """Trains learned fusion with FPR-constrained thresholding."""
    ...
    def build_feature_matrix(self) -> Tuple[np.ndarray, np.ndarray, List[Dict]]:
    ...
    def train_logistic_regression(self, X_train: np.ndarray, y_train: np.ndarray) -> Tuple[np.ndarray, float]:
        """
        Train logistic regression using numpy (no sklearn dependency).
    ...
    def find_threshold_for_fpr(self, y_true: np.ndarray, y_proba: np.ndarray, 
        ...
        sorted_proba = np.sort(benign_proba)[::-1]  # Descending
        target_fp_count = max(1, int(len(benign_proba) * target_fpr))
    ...
    def run_cv(self, X: np.ndarray, y: np.ndarray, n_splits: int = 5) -> Dict:
        """Run stratified 5-fold CV with FPR-constrained thresholding."""
        print("\n" + "="*70)
        ...
        for fold_id in range(n_splits):
            ...
            # Split data
            val_idx = np.array(fold_indices[fold_id])
            ...
            threshold = self.find_threshold_for_fpr(y_val, y_proba_val, target_fpr=0.01)
    ...
    def run(self):
        ...
        print("\n" + "="*70)
        print("PHASE 5: LEARNED FUSION TRAINING")
        print("="*70)
        ...
        X, y, sample_info = self.build_feature_matrix()
        ...
        cv_results['cv_results'].to_csv(
            self.results_dir / "learned_fusion_cv_metrics.csv", index=False
        )
        print(f"\n✓ Saved CV metrics to {self.results_dir / 'learned_fusion_cv_metrics.csv'}")
        
        # Save feature importance
        if self.feature_importance:
            df_importance = pd.DataFrame({
                'feature': self.feature_importance['features'],
                'coefficient': self.feature_importance['coefficients'],
            })
            ...
            df_importance.to_csv(
                self.results_dir / "learned_fusion_feature_importance.csv", index=False
            )
            print(f"✓ Saved feature importance to {self.results_dir / 'learned_fusion_feature_importance.csv'}")
        
        # Save thresholds
        df_thresholds = pd.DataFrame({
            'fold': range(1, len(cv_results['fold_thresholds']) + 1),
            'threshold': cv_results['fold_thresholds'],
        })
        df_thresholds.to_csv(
            self.results_dir / "learned_fusion_thresholds.csv", index=False
        )
        print(f"✓ Saved fold thresholds to {self.results_dir / 'learned_fusion_thresholds.csv'}")
        
        print("\n" + "="*70)
        print("✅ LEARNED FUSION TRAINING COMPLETE")
        print("="*70)
