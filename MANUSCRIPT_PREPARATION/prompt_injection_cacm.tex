...
\begin{itemize}
  \item We present a deployable input-side defense pipeline (``LLM firewall'') combining \emph{Normalization} + \emph{Signature} (rule-based pattern matching) + \emph{Semantic} (embedding-based similarity) detectors with \emph{OR-fusion}, achieving 87\% detection of known attacks with $<$1\% false alarms.
...
We assembled the complete pipeline (Normalizer → parallel v1+v3 detectors → OR-fusion) and profiled it on standard cloud hardware: an AWS EC2 t3.medium instance (2 vCPUs, 4 GB RAM) running Python 3.9. We measured 1,000 queries to capture realistic performance.
...
We profiled CPU, memory, and throughput under stress. Peak CPU utilization was 18\% on a single core (embedding computation is the bottleneck). Memory footprint was constant at 142\,MB for loaded artifacts (v1 rules, v3 embedding model, fusion weights). We stress-tested with 100 concurrent queries over 10,000 requests and observed linear scaling with no memory leaks.
...
\begin{figure}[t]
  ...
  \caption{Threshold invariance demonstration. Lines show TPR (green) and FAR (red) as v3's internal similarity threshold varies from 0.3 to 0.9. OR-fusion maintains stable 87\% TPR and 0\% FAR across all thresholds, eliminating the need for careful threshold tuning—a key operational advantage.}
