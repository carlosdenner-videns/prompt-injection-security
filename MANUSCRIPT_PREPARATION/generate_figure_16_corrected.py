#!/usr/bin/env python3
"""
Figure 16: System Architecture - Corrected Detector Alignment
Each detector (v1, v2, v3) in its own properly aligned box
"""
...
# Arrow to Detectors
arrow2 = FancyArrowPatch((4.3, y_pipeline - 0.05), (5.0, y_pipeline - 0.05), 
                         arrowstyle='->', mutation_scale=25, linewidth=2.5, color=color_border)
ax.add_patch(arrow2)
...
# ============================================================================
# These are positioned vertically, stacked
detector_width = 1.3
detector_height = 0.65
detector_x_start = 5.0
spacing_between = 0.75
...
# Arrow to Decision
arrow3 = FancyArrowPatch((fusion_x + 1.6, fusion_y), (9.2, fusion_y), 
                         arrowstyle='->', mutation_scale=25, linewidth=2.5, color=color_border)
ax.add_patch(arrow3)
...
# DECISION
# ============================================================================
decision_box = FancyBboxPatch((9.2, fusion_y - 0.45), 1.5, 1.0, boxstyle="round,pad=0.08",
                              edgecolor='#B71C1C', facecolor=color_decision, linewidth=2.5)
ax.add_patch(decision_box)
ax.text(9.95, fusion_y + 0.15, 'DECISION', ha='center', va='center', 
        fontsize=9, fontweight='bold', color='white')
ax.text(9.95, fusion_y - 0.15, 'Attack/Benign', ha='center', va='center', 
        fontsize=8, color='white')

# Arrow BLOCKED (downward)
arrow_blocked = FancyArrowPatch((9.95, fusion_y - 0.45), (9.95, fusion_y - 1.1), 
                                arrowstyle='->', mutation_scale=25, linewidth=2.5, 
                                color='#B71C1C', linestyle='--')
ax.add_patch(arrow_blocked)
ax.text(10.4, fusion_y - 0.75, 'BLOCKED', ha='left', fontsize=8, fontweight='bold', 
        color='#B71C1C')

# Arrow ALLOWED (rightward)
arrow_allowed = FancyArrowPatch((10.7, fusion_y), (11.7, fusion_y), 
                                arrowstyle='->', mutation_scale=25, linewidth=2.5, 
                                color='#2E7D32')
ax.add_patch(arrow_allowed)
ax.text(11.2, fusion_y + 0.35, 'ALLOWED', ha='center', fontsize=8, fontweight='bold', 
        color='#2E7D32')

# ============================================================================
# LLM PROCESSING
# ============================================================================
llm_box = FancyBboxPatch((11.7, fusion_y - 0.45), 1.5, 1.0, boxstyle="round,pad=0.08",
                         edgecolor='#F57F17', facecolor=color_llm, linewidth=2.5)
ax.add_patch(llm_box)
ax.text(12.45, fusion_y + 0.15, 'LLM', ha='center', va='center', 
        fontsize=9, fontweight='bold', color='#F57F17')
ax.text(12.45, fusion_y - 0.15, 'Processing', ha='center', va='center', 
...
print("✓ Figure 16 (Corrected - Detectors Aligned) generated successfully!")
print(f"  Saved to: {output_path}")
print("\nDetector Alignment Improvements:")
print("  ✓ v1 (Signature) in separate box - TOP")
print("  ✓ v2 (Heuristic) in separate box - MIDDLE")
print("  ✓ v3 (Semantic) in separate box - BOTTOM")
print("  ✓ Each text centered in its own box")
print("  ✓ Proper arrows from each detector to fusion")
print("  ✓ Clear visual separation")
print("  ✓ Professional alignment")
